...
českých autosémantik (tedy primárně substantiv, adjektiv, verba
...
lexémy z~korpusu (konkrétně šlo korpus řady SYN a~bylo vybráno přibližně
260 000 lemmat). Druhou otázkou bylo, jaký typ slovotvorného vztahu by
měl být v~derivační síti reflektován, v~tomto případě byla vybrána
derivace z~důvodu svého dominantního postavení v~české slovotvorbě,
nicméně je do budoucna plánováno popsat i~slovotvorné vztahy týkající se
skládání slov, to by ale kompletně pozměnilo architekturu sítě, kde má
každé základové slovo právě jen jeden odkaz na určitý derivát.
\parencite{sevcikova14}

Pro tvorbu samotné derivační sítě, resp. jednotlivých derivačních vztahů
bylo zapotřebí identifikovat dvojice slov základových a~odvozených -- to
bylo docíleno poloautomatickou metodou, kdy byly vyděleny takové dvojice
lemmat, v~rámci kterých měla obě slova dostatečně dlouhý společný
počáteční podřetězec a~zároveň se lišila specifickým koncovým
podřetězcem. Ze čtyř set nejfrekventovanějších pak bylo ručně
vyextrahováno osmnáct dvojic, které odpovídají popisovaným derivačním
vztahům v~české slovní zásobě, u~těchto dvojic byl taktéž určen směr
derivace (typicky bylo za základové slovo považováno to, které mělo
kratší základové lemma).~\parencite{derinet-cz}

Dalším způsobem, pomocí kterých šlo určovat derivační vztahy, bylo
extrakcí z~morfologického slovníku MorfLex CZ -- ten obsahuje 120
milionů trojic tvořených lemmatem, morfologickou značkou a~slovním
tvarem, příkladem může být trojice
\texttt{žížnivý\_,n;\ AAFS6-\/-\/-\/-3N-\/-\/-\/-;\ nejnežížnivější}
\parencite{morflex}. Jak je z~příkladu patrné, slovník obsahuje
informace určené primárně pro flektivní morfologii nicméně \ldots{}.
\parencite{sevcikova16}
